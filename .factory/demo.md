...
redaction, and receipt code. It creates a newly named directory under the
system temporary directory, writes two no-value receipts, prints the directory
path, and does not read the OS keychain or `ASC_HOME`. Delete the printed
directory to reset it. The shipped fixture explanation is `examples/agent-output.txt`.
